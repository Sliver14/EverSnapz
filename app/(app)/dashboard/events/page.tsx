  ...
  return (
    <div className="max-w-[1000px] mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-[32px] font-extrabold text-dark-text tracking-tighter m-0 leading-tight">My Events</h2>
          ...
          {events.map((event) => (
            <div key={event.id} className="bg-white p-6 rounded-2xl border border-border-color shadow-sm flex justify-between items-center group hover:border-primary-lilac transition-all">
              <div className="flex gap-5 items-center">
                <div className="w-16 h-16 bg-light-lavender rounded-xl flex items-center justify-center text-primary-lilac overflow-hidden shadow-sm">
                  {event.coverPhotoUrl ? (
                ...
                <div>
                  <h4 className="text-[18px] font-black text-dark-text m-0">{event.name}</h4>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-[12px] text-gray-text font-bold">/guest/{event.slug}</span>
